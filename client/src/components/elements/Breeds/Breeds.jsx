...
const Breed = () => {
  const getBreed = index => {
    BreedsServices.getBreed(index).then(elm => setBreed(elm)).catch(err => setBreed([err]))
  };
  ...
  return (
    <>
      ...
      <div>{breed && breed.map((elm, idx) => <><h3 key={elm._id}>{elm.name}</h3><button onClick={() => getDetailsBreed(elm)}>Detalles raza</button></>)}</div>
